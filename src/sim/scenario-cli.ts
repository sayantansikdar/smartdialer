...
function formatReport(report: SimulationReport): string {
  const lines = [
    ...
    '',
    row('Pacing requested (total)', report.safetyDecisions.totalRequested),
    row('Safety approved (total)', report.safetyDecisions.totalApproved),
    row('  approved as asked', report.safetyDecisions.approved),
    row('  reduced', report.safetyDecisions.reduced),
    row('  rejected', report.safetyDecisions.rejected),
    row('  fell back to progressive', report.safetyDecisions.fallbackToProgressive),
    '',
